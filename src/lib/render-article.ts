  ...
  const mediaHtml = media
    .map((m) => {
      ...
      if (tweet.videoUrl && m.kind === "gif") {
        return `<video src="${escapeHtml(tweet.videoUrl)}" controls class="w-full rounded-md mt-2" onerror="this.style.display='none';this.nextElementSibling.style.display='block'"></video><img src="${src}" alt="" class="w-full rounded-md mt-2" style="display:none" />`;
      }
      return `<img src="${src}" alt="" class="w-full rounded-md mt-2" />`;
    })
    .join("");
  return `<blockquote class="tweet-embed border border-(--color-border) rounded-lg bg-(--color-surface) p-3 my-3 max-w-md">
  <p class="font-semibold">${escapeHtml(tweet.authorName)} <span class="text-(--color-muted)">@${escapeHtml(tweet.authorHandle)}</span></p>
  <p class="mt-1 whitespace-pre-wrap">${escapeHtml(tweet.text)}</p>
  ${mediaHtml}
  <a href="${escapeHtml(tweet.sourceUrl)}" target="_blank" rel="noreferrer external" class="text-(--color-link) text-xs mt-2 inline-block">Ver en X</a>
</blockquote>`;
}
...
export function buildBookmarkCardHtml(bookmark: {
  ...
  const imageHtml =
    bookmark.imageData && bookmark.imageMimeType
      ? `<img src="data:${escapeHtml(bookmark.imageMimeType)};base64,${bookmark.imageData.toString("base64")}" alt="" class="w-full rounded-md mt-2" />`
      : "";
  const descriptionHtml = bookmark.description
    ? `<p class="mt-1 text-(--color-muted) text-sm">${escapeHtml(bookmark.description)}</p>`
    : "";
  return `<a href="${escapeHtml(bookmark.url)}" target="_blank" rel="noreferrer external" class="bookmark-embed block border border-(--color-border) rounded-lg bg-(--color-surface) p-3 my-3 max-w-md no-underline hover:shadow-sm">
  <p class="font-semibold text-(--color-link)">${faviconHtml}${escapeHtml(bookmark.title)}</p>
  ${descriptionHtml}
  ${imageHtml}
</a>`;
}
...
export async function renderArticleContent(source: string): Promise<string> {
  ...
  const html = renderMarkdown(withPlaceholders(source, bookmarkUrlByIndex));
